...
public class Scheduler implements Runnable {
	 ...
	 */
	public Scheduler(FloorSubsystem floorsub/* , ElevatorSubsystem subsys */) {
		...
		synchronized (this.floorsub.getInfo()) {
			while (true) {
				if (this.floorsub.getInfo().size() == 0) {
					System.out.println(LocalTime.now() + " Scheduler found that FloorSubsytem doesn't have any new jobs for it.");
